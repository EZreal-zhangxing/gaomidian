...
public class ArticelService {
	...
	{
		return articelDaoImp.getArticelbyid(articelId);
	}
	/**
	 * 关闭活动预约
	 */
	public Message closeArticel(String id)
	{
		int result=articelDaoImp.closeOrder(id);
		if(result>0)
		{
			return new Message("success", "关闭成功！", new Date());
		}else
		{
			return new Message("fail", "关闭失败！", new Date());
		}
		
	}
	public Message openArticel(String id)
	{
		int result=articelDaoImp.openOrder(id);
		if(result>0)
		{
			return new Message("success", "开启成功！", new Date());
		}else
		{
			return new Message("fail", "开启失败！", new Date());
		}
